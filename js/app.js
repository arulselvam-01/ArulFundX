...
import { searchFunds, fetchFundData, fetchBenchmarkData, fetchAllFundCodes } from './api.js';
import {
...
function init() {
  ...
  // Load benchmark in background (don't block UI)
  fetchBenchmarkData()
    .then(applyBenchmarkData)
...
async function loadFund(schemeCode, schemeName) {
  ...
  try {
    ...
    if (state.sortedBenchNav) {
      ...
      try {
        const bData = await fetchBenchmarkData();
        applyBenchmarkData(bData);
      } catch (_) {
...
function populateFundInfoStrip(fundData, sortedNav) {
  ...
  setText('fund-amc', meta.fund_house || '—');
...
function renderCompareAlphaTable() {
  ...
  setText('cmp-alpha-t-h-a', a.name);
  setText('cmp-alpha-t-h-b', b.name);

  if (!state.sortedBenchNav) {
    tbody.innerHTML = `<tr><td colspan="4" style="text-align:center;color:var(--text-muted);padding:32px">Benchmark (Nifty 50) data not available.</td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = years.map(y => {
    ...
    const fA = ra?.fundReturn ?? null;
    const fB = rb?.fundReturn ?? null;
    const bench = (ra || rb)?.benchReturn ?? null;
    const alphaValA = ra?.alpha ?? null;
    const alphaValB = rb?.alpha ?? null;
    const { classA: fundACls, classB: fundBCls } = cmpGreenWinnerOnly(alphaValA, alphaValB, false);

    return `<tr>
      <td><strong>${y}</strong></td>
      <td class="${fundACls}">${fmtPct(fA)}</td>
      <td>${fmtPct(bench)}</td>
      <td class="${fundBCls}">${fmtPct(fB)}</td>
    </tr>`;
  }).join('');
}
...
function applyBenchmarkData(data) {
  ...
  state.sortedBenchNav = sortNavChronological(data.data);
  if (state.sortedNav) {
    state.riskMetrics = computeRiskMetrics(state.sortedNav, state.sortedBenchNav);
...
function renderAlphaTab() {
  if (!state.sortedBenchNav) {
    showAlphaUnavailable('Benchmark data not available. Alpha analysis requires Nifty 50 proxy data.');
    return;
  }
...
async function startRankingFetchProcess() {
  ...
  try {
    ...
    if (!state.sortedBenchNav) {
      if (progressText) progressText.textContent = 'Fetching Nifty 50 benchmark...';
      const bData = await fetchBenchmarkData();
      applyBenchmarkData(bData);
    }
