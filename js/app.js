...
import {
  ...
  computeRollingStats,
  getRollingScopeBounds,
  filterRollingPeriodsByScope,
  filterRollingReturnsByScope,
  computeAlpha,
  computeAlphaStats,
  computeRiskMetrics,
...
function init() {
  ...
  syncRollingScopeButtons(state.rollingPeriodScope);
...
function saveSession() {
  try {
    const session = {
      ...
      rollingPeriodScope: state.rollingPeriodScope,
...
async function restoreSession() {
  ...
  if (session.rollingPeriodScope) {
    setRollingPeriodScope(session.rollingPeriodScope);
  }
...
function setupRollingWindowButtons() {
  document.querySelectorAll('#rolling-scope-btns .period-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      setRollingPeriodScope(btn.dataset.rollingScope || 'all');
...
function syncRollingScopeButtons(scope = state.rollingPeriodScope || 'all') {
  document.querySelectorAll('#rolling-scope-btns .period-btn, #cmp-rolling-scope-btns .period-btn').forEach(b => {
    const btnScope = b.dataset.rollingScope || b.dataset.cmpRollingScope || 'all';
    b.classList.toggle('active', btnScope === scope);
  });
}

function setRollingPeriodScope(scope) {
  state.rollingPeriodScope = scope || 'all';
  syncRollingScopeButtons(state.rollingPeriodScope);
}

function setupCompareRollingButtons() {
  document.querySelectorAll('#cmp-rolling-scope-btns .period-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      setRollingPeriodScope(btn.dataset.cmpRollingScope || 'all');
      if (state.compareA && state.compareB) renderCompareTab();
    });
  });
...
function renderRollingTab() {
  ...
  const scope = state.rollingPeriodScope || 'all';
  const bounds = getRollingScopeBounds(state.sortedNav, scope);

  let rollingData = computeRollingReturns(state.sortedNav, W);
  let periods = computeRollingPeriods(state.sortedNav, W);

  if (bounds) {
    rollingData = filterRollingReturnsByScope(rollingData, bounds);
    periods = filterRollingPeriodsByScope(periods, W, bounds);
  }

  const stats = computeRollingStats(periods);

  const scopeSuffix = bounds
    ? `, ${bounds.periodStart}–${bounds.periodEnd} completed years`
    : '';
  const scopePrefix = bounds ? 'Latest scope ' : 'All ';

  setText('rolling-chart-label', `(${scopePrefix}overlapping ${W}Y windows${scopeSuffix}, monthly sampled)`);
  setText('rolling-table-label', `(${scopePrefix}rolling ${W}Y windows${scopeSuffix}, yearly progression)`);
  ...
  if (rollingData.length) {
    ...
    if (canvas) {
      ...
      const chartMsg = bounds && !bounds.hasFullCoverage
        ? `Not enough history for latest ${scope === '5yr' ? '5' : '10'}Yr rolling returns`
        : `Not enough data for ${W}Y rolling returns`;
      ctx.fillText(chartMsg, canvas.width / 2, 60);
    }
  }
  ...
  if (!periods.length) {
    const scopeMsg = bounds && !bounds.hasFullCoverage
      ? `Not enough completed-year history for latest ${scope === '5yr' ? '5' : '10'}-year rolling analysis (needs data from ${bounds.periodStart} onward).`
      : `Not enough historical data for ${W}-year rolling periods.`;
    tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;color:var(--text-muted);padding:24px">${scopeMsg}</td></tr>`;
...
const RANKING_ROLLING_PERIOD_LABELS = {
  all: 'All Time',
  '5yr': 'Latest 5Yr',
  '10yr': 'Latest 10Yr',
};

function getRankingRollingScoreField(periodKey = state.rankingPeriodKey) {
  if (periodKey === '5yr') return 'rollingScore5yr';
  if (periodKey === '10yr') return 'rollingScore10yr';
  return 'rollingScore';
}

function getFundRollingScore(fund, periodKey = state.rankingPeriodKey) {
  if (!fund) return null;
  return fund[getRankingRollingScoreField(periodKey)];
}

async function preloadRankingLeaderboard() {
  // v6 — rolling 3Y consistency: all-time + latest 5Yr / 10Yr period scopes
  localStorage.removeItem('fl-ranked-funds-v2');
  localStorage.removeItem('fl-ranked-funds-v3');
  localStorage.removeItem('fl-ranked-funds-v4');
  localStorage.removeItem('fl-ranked-funds-v5');
  const stored = localStorage.getItem('fl-ranked-funds-v6');
  if (stored) {
    const parsed = JSON.parse(stored);
    const sample = parsed[0];
    const hasPeriodScores = sample
      && sample.rollingScore != null
      && 'rollingScore5yr' in sample
      && 'rollingScore10yr' in sample;
    if (Array.isArray(parsed) && parsed.length > 0 && hasPeriodScores) {
      state.rankedFunds = parsed;
      sortLeaderboard();
    } else {
      localStorage.removeItem('fl-ranked-funds-v6');
      state.rankedFunds = [];
    }
  } else {
    state.rankedFunds = [];
...
function persistRankedFunds() {
  localStorage.setItem('fl-ranked-funds-v6', JSON.stringify(state.rankedFunds));
}

function sortLeaderboard() {
  const field = getRankingRollingScoreField();
  state.rankedFunds.sort((a, b) => (b[field] || 0) - (a[field] || 0));
}

/** Average of every 3Y rolling window fully inside [periodStart, periodEnd]. */
function average3YRollingInPeriod(yearFirstNavMap, yearNavMap, periodStart, periodEnd) {
  const windowCAGRs = [];
  for (let startY = periodStart; startY + 2 <= periodEnd; startY++) {
  ...
  return windowCAGRs.length
    ? windowCAGRs.reduce((s, v) => s + v, 0) / windowCAGRs.length
    : null;
}

/**
 * Compute rolling 3-year CAGR consistency scores for ranking.
 ...
 *
 * All-time: average of every 3Y rolling window across full history.
 * Latest 5Yr / 10Yr: same 3Y rolling method, but only windows inside the latest
 * 5 or 10 completed calendar years (e.g. 2020–2025 when current year is 2026).
 */
function computeRollingScores(sortedNav) {
  ...
  const fundAge   = lastYear - firstYear;
  ...
  for (const item of completedNav) {
    ...
    yearNavMap[y] = parseFloat(item.nav);
  }
  ...
  const rollingScore = average3YRollingInPeriod(yearFirstNavMap, yearNavMap, firstYear, lastYear);
  if (rollingScore === null) return null;

  const latest5Start = lastYear - 5;
  const latest10Start = lastYear - 10;

  const rollingScore5yr = firstYear <= latest5Start
    ? average3YRollingInPeriod(yearFirstNavMap, yearNavMap, latest5Start, lastYear)
    : null;

  const rollingScore10yr = firstYear <= latest10Start
    ? average3YRollingInPeriod(yearFirstNavMap, yearNavMap, latest10Start, lastYear)
    : null;

  return { rollingScore, rollingScore5yr, rollingScore10yr };
}

function computeRollingScore(sortedNav) {
  const scores = computeRollingScores(sortedNav);
  return scores ? scores.rollingScore : null;
}
...
function addFundToRanking(schemeCode, schemeName, category, sortedNav) {
  ...
  const rollingScores = computeRollingScores(sortedNav);
  if (rollingScores === null) return; // Does not qualify
...
function setupRankingControls() {
  ...
  const periodSelect = document.getElementById('ranking-period-select');
  if (periodSelect) {
    periodSelect.value = state.rankingPeriodKey || 'all';
    periodSelect.addEventListener('change', () => {
      state.rankingPeriodKey = periodSelect.value;
      state.rankingDisplayLimit = 100;
      sortLeaderboard();
      renderRankingTab();
    });
...
function updateRankingPeriodLabels() {
  ...
  const periodLabel = RANKING_ROLLING_PERIOD_LABELS[periodKey] || 'All Time';
  ...
  const rollingHeader = document.getElementById('ranking-rolling-header');

  let periodDetail;
  if (periodKey === '5yr') {
    periodDetail = `average of 3‑year rolling window CAGRs within the latest 5 completed years (${lastCompletedYear - 5}–${lastCompletedYear})`;
  } else if (periodKey === '10yr') {
    periodDetail = `average of 3‑year rolling window CAGRs within the latest 10 completed years (${lastCompletedYear - 10}–${lastCompletedYear})`;
  } else {
    periodDetail = 'average of all 3‑year rolling window CAGRs across full history';
  }

  if (introDesc) {
    introDesc.innerHTML = `All Direct Growth mutual funds ranked by <strong>${periodLabel} Rolling 3‑Year CAGR Consistency</strong> — ${periodDetail}. Completed calendar years only (current year excluded). Only funds with at least 5 years of history are ranked.`;
  }
  if (tableLabel) {
    tableLabel.textContent = `(Ranked by ${periodLabel} Rolling 3Y CAGR Consistency)`;
  }
  if (rollingHeader) {
    rollingHeader.textContent = `${periodLabel} Rolling Return`;
  }
}
...
function renderRankingTab() {
  ...
  const periodKey = state.rankingPeriodKey || 'all';
  const scoreField = getRankingRollingScoreField(periodKey);
  ...
  const filteredFunds   = activeFunds.filter(fund => {
    ...
    if (periodKey !== 'all' && fund[scoreField] == null) return false;
    return true;
  });
...
async function renderCompareTab() {
  ...
  import('./charts.js').then(m => {
    ...
    // Rolling Chart & Table
    const W = state.rollingWindow || 3;
    const scope = state.rollingPeriodScope || 'all';
    const boundsA = getRollingScopeBounds(a.sortedNav, scope);
    const boundsB = getRollingScopeBounds(b.sortedNav, scope);

    let rollA = computeRollingPeriods(a.sortedNav, W);
    let rollB = computeRollingPeriods(b.sortedNav, W);
    if (boundsA) rollA = filterRollingPeriodsByScope(rollA, W, boundsA);
    if (boundsB) rollB = filterRollingPeriodsByScope(rollB, W, boundsB);

    m.renderCompareRollingChart(rollA, rollB, W, a.name, b.name);

    const scopeBounds = boundsA || boundsB;
    const scopeSuffix = scopeBounds
      ? `, ${scopeBounds.periodStart}–${scopeBounds.periodEnd} completed years`
      : '';
    const scopePrefix = scopeBounds ? 'latest scope ' : '';
    setText('cmp-rolling-label', `(${scopePrefix}${W}Y windows${scopeSuffix}, yearly progression)`);
    ...
    if (cmpRollingTbody) {
      ...
      if (!sortedPeriods.length) {
        const scopeBounds = boundsA || boundsB;
        const scopeMsg = scopeBounds && !(boundsA?.hasFullCoverage && boundsB?.hasFullCoverage)
          ? `Not enough completed-year history for latest ${scope === '5yr' ? '5' : '10'}-year rolling comparison.`
          : `Not enough history for ${W}Y rolling periods.`;
        cmpRollingTbody.innerHTML = `<tr><td colspan="3" style="text-align:center;color:var(--text-muted);padding:16px">${scopeMsg}</td></tr>`;
