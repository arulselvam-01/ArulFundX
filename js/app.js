...
function renderCompareRiskTable() {
  ...
  const { breakdown, averageA, averageB, windowCount } = computeCompareRiskMetricsBreakdown(
    a.sortedNav,
    b.sortedNav,
  ...
  setText('cmp-risk-label', spec
    ? `(avg. across ${windowCount} overlapping ${spec.shortLabel} windows)`
    : '');
  ...
  if (tbody) {
    tbody.innerHTML = CMP_RISK_SUMMARY_METRICS.map(rm => {
      const valA = cmpRiskMetricVal(averageA, rm);
      const valB = cmpRiskMetricVal(averageB, rm);
...
function renderRiskTab() {
  ...
  const { breakdown, averageMetrics, gaugeMetrics, windowCount } =
  ...
  if (captionEl) {
    if (windowCount > 0) {
      captionEl.textContent = `Gauges: Average across ${windowCount} overlapping ${spec?.shortLabel || windowKey} windows`;
    } else {
  ...
  // Drawdown stat reflects average max drawdown; chart shows full history
  const ddVal = averageMetrics?.maxDrawdown ?? gaugeMetrics?.maxDrawdown;
  if (ddVal !== null && isFinite(ddVal)) {
    setText('drawdown-value', fmtPct(ddVal));
    if (windowCount > 1) {
      setText('drawdown-period', `  (avg. across ${windowCount} windows)`);
    } else {
