...
/** Latest completed-year scope for rolling analysis (current calendar year excluded). */
export function getRollingScopeBounds(sortedNav, scopeKey = 'all') {
  if (!sortedNav?.length || scopeKey === 'all') return null;

  const span = scopeKey === '5yr' ? 5 : scopeKey === '10yr' ? 10 : null;
  if (!span) return null;

  const currentYear = new Date().getFullYear();
  const completedNav = sortedNav.filter(n => parseNavDate(n.date).getFullYear() < currentYear);
  if (!completedNav.length) return null;
  ...
  const periodStart = lastYear - span;
  const periodEnd = lastYear;

  return {
    periodStart,
    periodEnd,
    hasFullCoverage: firstYear <= periodStart,
  };
}

export function filterRollingPeriodsByScope(periods, windowYears, bounds) {
  if (!bounds || !periods?.length) return periods || [];
  return periods.filter((p) => {
    const startY = parseNavDate(p.startDate).getFullYear();
    const endY = startY + windowYears - 1;
    return startY >= bounds.periodStart && endY <= bounds.periodEnd;
  });
}

export function filterRollingReturnsByScope(rollingData, bounds) {
  if (!bounds || !rollingData?.length) return rollingData || [];
  return rollingData.filter((r) => {
    const startY = parseNavDate(r.startDate).getFullYear();
    const endY = parseNavDate(r.endDate).getFullYear();
    return startY >= bounds.periodStart && endY <= bounds.periodEnd;
  });
}
...
export function computeRiskMetricsBreakdown(sortedFundNav, sortedBenchNav, windowKey) {
  ...
  const latestWindow = breakdown.length ? breakdown[breakdown.length - 1] : null;

  return {
    breakdown,
    averageMetrics,
    gaugeMetrics: latestWindow ? latestWindow.metrics : null,
    gaugePeriodLabel: latestWindow ? latestWindow.periodLabel : 'N/A',
...
export function computeCompareRiskMetricsBreakdown(sortedFundA, sortedFundB, sortedBenchNav, windowKey) {
  ...
  return {
    ...
    averageA,
    averageB,
    latestA: latestWindow ? latestWindow.metricsA : null,
    latestB: latestWindow ? latestWindow.metricsB : null,
    latestPeriodLabel: latestWindow ? latestWindow.periodLabel : 'N/A',
    windowCount: breakdown.length,
  };
}
