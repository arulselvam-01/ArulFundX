...
/**
 * Compute NON-OVERLAPPING rolling periods (for the breakdown table)
 * e.g. for 3Y: 2005-2008, 2008-2011, 2011-2014, ...
 */
export function computeRollingPeriods(sortedNav, windowYears) {
  ...
  while (true) {
    ...
    periodEnd.setFullYear(periodEnd.getFullYear() + windowYears);
    ...
    if (startItem && endItem && parseNavDate(startItem.date) < parseNavDate(endItem.date)) {
      ...
      if (cagr !== null && isFinite(cagr)) {
        const sy = parseNavDate(startItem.date).getFullYear();
        const ey = parseNavDate(endItem.date).getFullYear();
        results.push({
          periodLabel: `${sy}–${ey}`,
          startDate: startItem.date,
          endDate: endItem.date,
    ...
    // Next period starts where this one ends
