...
export function renderAlphaChart(alphaData) {
  ...
  opts.plugins.tooltip.callbacks = {
    label: item => {
      ...
      if (item.datasetIndex === 1) return ` Nifty 50: ${item.raw >= 0 ? '+' : ''}${item.raw.toFixed(2)}%`;
...
export function renderCompareAlphaChart(alphaA, alphaB, nameA, nameB) {
  ...
  const dataA = years.map(y => alphaA.find(r => r.year === y)?.fundReturn ?? null);
  const dataB = years.map(y => alphaB.find(r => r.year === y)?.fundReturn ?? null);
  const dataIndex = years.map(y => (alphaA.find(r => r.year === y) || alphaB.find(r => r.year === y))?.benchReturn ?? null);
  ...
  instances.cmpAlpha = new Chart(canvas, {
    ...
    data: {
      ...
      datasets: [
        { label: nameA, data: dataA, backgroundColor: t.blue, borderRadius: 4 },
        { label: nameB, data: dataB, backgroundColor: t.teal, borderRadius: 4 },
        { label: 'Nifty 50 Index', data: dataIndex, backgroundColor: 'rgba(156, 163, 175, 0.4)', borderRadius: 4, type: 'bar' }
      ]
    },
    options: opts
  });
...
export function renderCompareRollingChart(rollingA, rollingB, windowYears, nameA, nameB) {
  ...
  // Align dates from both funds
  const dateMap = new Map();
  rollingA.forEach(r => dateMap.set(r.endDate, { a: r.cagr, b: null }));
  rollingB.forEach(r => {
    if (dateMap.has(r.endDate)) dateMap.get(r.endDate).b = r.cagr;
    else dateMap.set(r.endDate, { a: null, b: r.cagr });
  });

  const sortedDates = [...dateMap.keys()].sort((d1, d2) => {
    const [d, m, y] = d1.split('-').map(Number);
    const [d_, m_, y_] = d2.split('-').map(Number);
    return new Date(y, m - 1, d) - new Date(y_, m_ - 1, d_);
  });

  const sample = sortedDates.filter((_, i) => i % (Math.max(1, Math.floor(sortedDates.length / 50))) === 0);
  const dataA = sample.map(d => dateMap.get(d).a);
  const dataB = sample.map(d => dateMap.get(d).b);
  ...
  instances.cmpRolling = new Chart(canvas, {
    type: 'line',
    data: {
      labels: sample.map(d => d.split('-').slice(1).join('-')),
      datasets: [
        { label: nameA, data: dataA, borderColor: t.blue, borderWidth: 2, tension: 0.3, pointRadius: 0, fill: false },
        { label: nameB, data: dataB, borderColor: t.teal, borderWidth: 2, tension: 0.3, pointRadius: 0, fill: false }
      ]
    },
    options: opts
  });
